...
CONFLICT_FILE=$(find "$TEST_WS2/project" -name 'README.conflict-remote-*.md' -print -quit)
...
DELETE_CONFLICT=$(find "$TEST_WS2/project" -name 'README.conflict-local-*.md' -print -quit)
test -n "$DELETE_CONFLICT"
test "$(cat "$DELETE_CONFLICT")" = "local delete-conflict edit"
$DEVDROP conflicts "$TEST_WS2" | grep -q "README.conflict-local"
